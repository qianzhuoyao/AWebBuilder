...
import { genLogicConfigMap, IFormConfigInfo } from '../../../Logic/nodes/logicConfigMap.ts';
...
const CodeTip = memo(() => {
  ...
  const defaultValue = genLogicConfigMap().configInfo.get(logicState.target[0])?.formConfigInfo?.json;
  const height = useAutoHeight();
  const [code, setCode] = useState(defaultValue);
  const { theme } = useTheme();

  const updateCode = useCallback((newObj: object) => {
    console.log(newObj, 'sssnewObj');
    setCode(newObj);
...
const CodeEdit = memo(() => {
  ...
  const height = useAutoHeight();
  const defaultCode = genLogicConfigMap().configInfo.get(logicState.target[0])?.formConfigInfo?.json;
  const [errorMsg, setErrorMsg] = useState('');

  const [code, setCode] = useState(defaultCode);
  const { theme } = useTheme();

  const updateEditCode = useCallback((value: string) => {
    console.log(value, 'sssnewsObj');
    try {
      setErrorMsg('');
...
const FormMerge = memo(() => {
  ...
  const defaultCheck = genLogicConfigMap().configInfo.get(logicState.target[0])?.formConfigInfo?.mergePre;
  const [check, setCheck] = useState(defaultCheck);

  const updateCheck = useCallback((value: boolean) => {
    setCheck(value);
    genLogicConfigMap().configInfo.set(
      logicState.target[0],
      {
        formConfigInfo: {
          ...(genLogicConfigMap().configInfo.get(logicState.target[0])?.formConfigInfo as IFormConfigInfo),
          mergePre: value,
        },
      },
    );
  ...
  return <>
    ...
    <div className={'flex items-center justify-between mb-1'}>
      <small className={'flex items-center'}>
        <Tooltip content={<div className={'w-[200px]'}>
...
const FormContentConfig = () => {
  return <>
    <Tabs aria-label="Dynamic FormContentConfigtabs"
          classNames={{
            panel: 'p-1',
            base: 'px-1',
          }}>
    ...
    if (nodeInfo.target.length > 0) {
      return <>
        <div className="flex w-full flex-col px-1">
          <Tabs aria-label="Dynamic FormConfigtabs"
                classNames={{
                  panel: 'p-1',
                  base: 'px-1',
                }}>
