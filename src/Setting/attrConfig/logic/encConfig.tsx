...
import { Card, CardBody, Input, Select, Selection, SelectItem, Tab, Tabs } from '@nextui-org/react';
import { memo, useCallback, useState } from 'react';
import { ENCRYPTION_METHODS, genLogicConfigMap, IEncryptionConfigInfo } from '../../../Logic/nodes/logicConfigMap.ts';
...
const SetEncC = memo(() => {
  ...
  const defaultConfig =
    genLogicConfigMap().configInfo.get(logicState.target[0])?.encryptionConfigInfo;


  const [publicKey, setPublicKey] = useState(defaultConfig?.publicKey || '');
  const [encMethods, setEncMethods] = useState<typeof ENCRYPTION_METHODS[number]>(defaultConfig?.encryptionMethod || 'MD5');

  const onHandleChangEnc = useCallback((key: Selection) => {
    setEncMethods([...key][0] as typeof ENCRYPTION_METHODS[number]);
    genLogicConfigMap().configInfo.set(
      logicState.target[0],
      {
        encryptionConfigInfo: {
          ...genLogicConfigMap().configInfo.get(logicState.target[0])?.encryptionConfigInfo as IEncryptionConfigInfo,
  ...
  const updatePublicKey = useCallback((value: string) => {
    setPublicKey(value);
    genLogicConfigMap().configInfo.set(
      logicState.target[0],
      {
