...
import { INs, pix_Table, updateInstance } from '../../../store/slice/nodeSlice.ts';
...
const TakeForm = memo(() => {
  ...
  const onChangeConfig = useCallback((value: string, key: keyof ITableConfig) => {
  ...
  const updateTableConfig = useCallback(() => {
    insertConfig(NodesState.targets[0], {
      colProp: NodesState.list[NodesState.targets[0]]?.instance?.option?.colProp,
      colLabel: NodesState.list[NodesState.targets[0]]?.instance?.option?.colLabel,
      colField: NodesState.list[NodesState.targets[0]]?.instance?.option?.colField,
      dataField: NodesState.list[NodesState.targets[0]]?.instance?.option?.dataField,
    });
  }, [NodesState.list, NodesState.targets]);
  ...
  return <div>
    <div>
      ...
      <Input
        ...
        placeholder="表头取值"
        labelPlacement="outside"
        value={NodesState.list[NodesState.targets[0]]?.instance?.option?.colField || ''}
        onChange={e => {
    ...
    <div>
      ...
      <Input
        type="text"
        value={NodesState.list[NodesState.targets[0]]?.instance?.option?.colLabel || ''}
        placeholder="表头label"
        labelPlacement="outside"
        onChange={e => {
    ...
    <div>
      ...
      <Input
        type="text"
        value={NodesState.list[NodesState.targets[0]]?.instance?.option?.colProp || ''}
    ...
    <div>
      ...
      <Input
        type="text"
        value={NodesState.list[NodesState.targets[0]]?.instance?.option?.dataField || ''}
        placeholder="内容取值"
        labelPlacement="outside"
        onChange={e => {
      ...
      return <>
        <div className="flex w-full flex-col px-1">
          <Tabs aria-label="chart config" classNames={{
            ...
            <Tab key={nodeInfo.target[0] + 'TableConfigSetting'} title={'表格配置'}>
              <Card>
                <CardBody>
                  <TableConfigSetting id={nodeInfo.target[0]}></TableConfigSetting>
                </CardBody>
              </Card>
            </Tab>
