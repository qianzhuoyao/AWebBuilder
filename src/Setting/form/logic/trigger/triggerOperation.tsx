...
export const TriggerOperation = memo(
  ({ go, target }: { go?: () => void; target: string }) => {
    ...
    return (
      <>
        <Button size={"sm"} onClick={handleClick} color="primary">
          发送
        </Button>
        <div className={"mt-1"}>
          {/*<small>总发送次数:{*/}
          ...
          <div className={"flex justify-around mt-1"}>
            <Tooltip color={"default"} content={"总发送次数"} className="">
              <p className="text-small text-default-500 truncate">
                ...
                <p className={"text-center"}>
                  {Object.keys(logicState.sendDugCount[target] || []).length}
                </p>
              </p>
            </Tooltip>
            ...
            <Tooltip color={"default"} content={"总成功次数"} className="">
              <p className="text-small text-default-500 truncate">
                ...
                <p className={"text-center"}>
                  {
                    Object.values(logicState.sendDugCount[target] || []).filter(
                      (i) => i.type === "success"
                    ).length
                  }
                </p>
        ...
        <div className={"mt-1"}>
          ...
          <div
            className={"mt-1 overflow-y-scroll px-[5px]"}
            style={{
              height: height - 220 + "px",
            }}
          >
            {Object.keys(logicState.sendDugCount[target] || []).map(
              (key, index) => {
                return (
                  <div className={"my-1"} key={index}>
                    <TaskCard
                      id={key}
                      target={target}
                      item={logicState.sendDugCount[target][key]}
                    ></TaskCard>
                  </div>
                );
              }
