import { genLogicConfigMapToJSON, genLogicConfigMapToParse } from '../Logic/nodes/logicConfigMap.ts';
...
import { getLayerContentToJSON, getLayerContentToParse } from '../panel/layers.ts';
 ...
 */
export const toParseInPanel = (data: IParseInPanel, panelViewPaint: {
...
export const messageEventListener = <T, >(callback: (params: T) => void) => {
  const subscription = (e: MessageEvent<{
    ...
    if (e.origin === window.location.protocol + '//' + window.location.hostname + ':' + SERVICE_PORT) {
