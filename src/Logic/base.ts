...
interface IMenu<Pre, F, I, O> {
  ...
  initLogicInMake: Map<string, (v: IMessageFromStream<Pre, F>) => Observable<I>>;
  initLogicOutMake: Map<string, (v: IMessageFromStream<Pre, F>) => Observable<O>>;
}

const createLogicMenuInstance = <Pre, F, I, O>(initializer: () => IMenu<Pre, F, I, O>): (() => IMenu<Pre, F, I, O>) => {
...
export const signalLogicNode = <F, I, O>({
  ...
  const signalIn = <Pre, >(portName: string, buildInPort: (v: IMessageFromStream<Pre, F>) => Observable<I>) => {
  ...
  const signalOut = <Pre, >(portName: string, buildOutPort: (v: IMessageFromStream<Pre, F>) => Observable<O>) => {
