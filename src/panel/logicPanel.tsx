    ...
    (params: ItemParams) => {
      GRef.current.G?.getEdges().map((edge) => {
        if (
          ...
          removePEM(
            edge.getTargetNode()?.getProp().nodeGId + edge.getTargetPortId() ||
              "",
            (edge.getSourcePortId() || "") + edge.getTargetPortId()
          );
        } else if (
          ...
          removePEM(
            edge.getSourceNode()?.getProp().nodeGId + edge.getSourcePortId() ||
              "",
            (edge.getSourcePortId() || "") + edge.getTargetPortId()
          );
        }
      });
      getWDGraph().removeVertex(params.props.nodeProp.nodeGId);
        ...
        connecting: {
          validateEdge(args) {
            ...
            if (
              ...
              (args.edge.getTargetPortId()?.split("#")[0] || "").indexOf("in") >
                -1
            ) {
              try {
                console.log(
                ...
                if (
                  ((args.edge.getTargetPortId() || "").indexOf(
                    ...
                    [
                      ...(findPEM(
                        args.edge.getTargetNode()?.getProp().nodeGId +
                          args.edge.getTargetPortId() || ""
                      ) || []),
                    ].length) ||
                  ((args.edge.getTargetPortId() || "").indexOf(
                    "logic_or_BOTH_get@in-or"
                  ) > -1 &&
                    [
                      ...(findPEM(
                        args.edge.getTargetNode()?.getProp().nodeGId +
                ...
                updateConnectStatus(
                  args.edge.getSourceNode()?.getProp().nodeGId || "",
                  1
                );
                updateConnectStatus(
                  args.edge.getTargetNode()?.getProp().nodeGId || "",
